...
class Inscripcion(controllers.Controller):
    ...
    def pendientes(self, asamblea):
        ...
        asamblea = model.Asamblea.get(asamblea)
        inscripciones = model.Inscripcion.selectBy(asamblea=asamblea,
                                                   enviado=False)
    ...
    def pendientesDepartamento(self, asamblea, departamento):
        ...
        asamblea = model.Asamblea.get(asamblea)
        departamento = model.Departamento.get(departamento)
        viaticos = model.Municipio.selectBy(
            departamento=departamento).throughTo.viaticos
    ...
    def enviarDepartamento(self, asamblea, departamento):
        ...
        flash(
            u'Marcadas como enviadas todas las Inscripciones de {0} en {1}'.format(
                asamblea.nombre, departamento.nombre))
...
class Asamblea(controllers.Controller):
    ...
    def confirmar(self, afiliado, asamblea):
        ...
        deshabilitado, msg, afiliado, banco, asamblea = self.confirmacion(
            asamblea, afiliado)
    ...
    def departamento(self, departamento, asamblea):
        ...
        return dict(municipios=municipios,
                    asamblea=model.Asamblea.get(asamblea),
                    departamento=model.Departamento.get(departamento))
    ...
    def ach(self, asamblea, banco):
        ...
        for inscripcion in inscripciones:
            if inscripcion.afiliado.multisolvent(2013,
                                                 gracia=True) and inscripcion.afiliado.banco != banco.id:
    ...
    def asistentesDepto(self, asamblea, departamento):
        ...
        viaticos = model.Municipio.selectBy(
            departamento=departamento).throughTo.viaticos
        inscripciones = viaticos.throughTo.inscripciones.filter(
            model.Inscripcion.q.asamblea == asamblea).orderBy('afiliado_id')
    ...
    def enviarMasa(self, asamblea):
        ...
        for inscripcion in inscripciones:
            inscripcion.enviada = True

        flash(u'Marcadas como enviadas todas las Inscripciones de {0}'.format(
            asamblea.nombre))
